...
const TYPE_VALUE_RESOLVER_MAP = {
  ...
  weekrange: {
    formatter(value, format) {
      if (Array.isArray(value) && value.length === 2) {
        return [
          WEEK_FORMATTER(value[0], format),
          WEEK_FORMATTER(value[1], format),
        ];
      }
      return "";
    },
    parser(value, format) {
      if (Array.isArray(value) && value.length === 2) {
        return [
          TYPE_VALUE_RESOLVER_MAP.date.parser(value[0], format),
          TYPE_VALUE_RESOLVER_MAP.date.parser(value[1], format),
        ];
      }
      return [];
    },
...
export default {
  ...
  computed: {
    ...
    parsedValue() {
      const yearStartIndex = this.valueFormat
        ? this.valueFormat.indexOf("yyyy")
        : -1;
      const weekStartIndex = this.valueFormat
        ? this.valueFormat.indexOf("WW")
        : -1;
      const weekStartIndex2 = this.valueFormat
        ? this.valueFormat.indexOf("W")
        : -1;
      if (
        this.value &&
        this.type === "week" &&
        ...
        const year = parseInt(
          this.value.substring(yearStartIndex, yearStartIndex + 4)
        );
        const week = parseInt(
          this.value
        ...
        return this.value.map((date) => {
          const year = parseInt(
            date.substring(yearStartIndex, yearStartIndex + 4)
          );
          const week = parseInt(
            date
              .substring(weekStartIndex > -1 ? weekStartIndex : weekStartIndex2)
